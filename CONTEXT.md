...
- Current-head T03 real regression was rebuilt from `基于知识图谱的云南植物知识问答系统的研究与构建_刘金平.pdf` with `benchmarks/real_products/liujinping_defense.content.json` into `/tmp/academic-ppt-t03-current-v2`. All 18 pages pass composition QA, object QA, and 7 structural checks. Fallback inspection of the full contact sheet and P007/P013/P016 confirms the stale transition text is absent, the five-model comparison remains a four-module composition, and the entity-recognition identifier is intact. It remains an unaccepted candidate for the same missing authoritative-render, full-resolution review, and user-confirmation gates.
...
- T05 (`红色通用`) is also semantically compiled from its five-page standard editable PPTX. `assets/template_specs/T05_red_general.semantic.json` records 10 semantic modules and 27 child slots: a research cover, problem-plus-primary-evidence page, three-stage research route, result/interpretation carrier, and ending. Its semantic spec validator and unbound standard-template object QA pass; the latter correctly notes that media provenance, empty-slot, font, and residue checks need a bound delivery manifest.
- Keep T05 `semantic_compilation_in_progress`, `compiled_not_release_accepted`, and `product_accepted=false`. It needs a real-source deck regression, bound object QA, authoritative Windows PowerPoint export, and full-resolution review. T06-T08 remain pending semantic compilation, so release acceptance is still blocked.
...
- The corrected T06 v3 candidate is `/tmp/academic-ppt-t06-current-v3`. Role-aware explanation-slot assignment now binds only text within the slot's stated font range; the 18-page candidate passes bound object QA, structural QA, scientific-semantic QA, composition QA, and editable-manifest QA. Its fallback contact sheet and P007/P013/P016 were inspected without an obvious overflow or content-association defect. This is still not formal acceptance: `find_powerpoint() == False`, its visual tasks remain `bound_to_slide`, and it lacks authoritative Windows PowerPoint export, explicit full-resolution visual review, and user confirmation.
- Native templates can place their visual identity in non-text editable color blocks rather than in slide backgrounds. Grammar extraction now identifies only large edge/full-canvas solid-fill shapes as `fixed_background_layers`; they become protected template-identity components, never content-module ownership. Dynamic rendering redraws these layers before all content for every scaffold mode. T06 v4 (`/tmp/academic-ppt-t06-current-v4`) confirms the purple full-cover/left-sidebar layers survive an 18-page real build while structural, composition, manifest, and bound object QA all pass.
- T07 P04 receives the same explicit 2/4/5-module complete-reconstruction contract as the geometric-equivalent T06 P04. The real 18-page graduation-defense candidate at `/tmp/academic-ppt-t07-current-v1` passes structural, scientific-semantic, composition, editable-manifest, and bound object QA. Its fallback contact sheet and P007/P013/P016 were inspected without an obvious overflow or content-association defect; the red full-cover/left-sidebar background identity is retained. Keep it unaccepted until authoritative Windows PowerPoint export, explicit full-resolution visual review, and user confirmation occur.
- T08 P04 now carries the same explicit 2/4/5-module reconstruction contract. The 18-page graduation-defense candidate at `/tmp/academic-ppt-t08-current-v1` passes structural, scientific-semantic, composition, editable-manifest, and bound object QA. Fallback contact-sheet and P007/P013/P016 inspection found no obvious overflow or content-association defect, and the blue full-cover/left-sidebar identity is retained. It remains an unaccepted candidate pending authoritative Windows PowerPoint export, explicit full-resolution visual review, and user confirmation.

## T05 Real-Source Continuation (2026-07-29)

- T05 P04 now explicitly authorizes complete-container reconstruction for 2, 4, or 5 visible modules, matching the equivalent top-navigation T04 and sidebar T06-T08 result/interpretation archetypes. The declaration is present in both the reusable T05 prototype and its standard template specification, so an exact native module match remains the default and only the named result container may rebuild at those counts.
- The real 18-page graduation-defense candidate at `/tmp/academic-ppt-t05-current-v1` uses `基于知识图谱的云南植物知识问答系统的研究与构建_刘金平.pdf` and `benchmarks/real_products/liujinping_defense.content.json`. Structural, scientific-semantic, composition, editable-manifest, and bound object QA pass. Eight pages record authorized complete-container reconstruction; no duplicate, orphan, empty-media, residue, font-bound, provenance, or exclusive-render-mode failure is reported.
...
- The separate ten-scene contract matrix at `/tmp/academic-ppt-scene-contract-matrix-v1/suite_result.json` passes 10/10 declared scene contracts. It intentionally uses synthetic fixtures and reports `product_suite_passed=false`; the result proves scene-profile coverage and must not be represented as a real-source product release matrix.
- Automatic source selection now maps scene-facing roles to evidence roles before ranking. In particular, weekly-progress roles map to background, contribution, experiment, result, limitation, and next-action evidence; project-competition roles map problem, user scenario, solution/technology, validation/advantage, application value/team, and roadmap to the corresponding research roles. This prevents an unmapped scene label from silently selecting an arbitrary unused source passage.
- The real-source project-competition compatibility candidate at `/tmp/academic-ppt-project-competition-t04-v2` builds 8 pages from `基于知识图谱的云南植物知识问答系统的研究与构建_刘金平.pdf` with scene `科研项目比赛` and T04. Structural, scene-semantic, composition, editable-manifest, and bound object QA pass. Its fallback preview flags several dense automatic-source pages, so it is an automated compatibility candidate only, not a human-quality or product-accepted deck. It also remains blocked by authoritative PowerPoint rendering, full-resolution review, and user confirmation.
- `组会-周报进展` now rejects a source that lacks explicit weekly cadence, current work state, blocker/decision, or next-action evidence. The attempted thesis-source run at `/tmp/academic-ppt-weekly-progress-t01-v1` was therefore correctly superseded: it had mechanical QA but did not truthfully support a weekly-report claim. Rebuilding as v2 fails early with a diagnostic rather than producing that misleading deck. A real weekly benchmark remains pending a source that contains the required status evidence.
- Windows PowerPoint is installed at `C:\Program Files\Microsoft Office\root\Office16\POWERPNT.EXE`. The unavailable-runtime status in this WSL session was caused by a Linux process and a failing WSL-to-Windows interop socket, not absence of Office. Forced PowerPoint preview and COM-control scripts now report that native Windows Python is required when run outside Windows, avoiding the previous misleading “not installed” message. The authoritative render remains pending execution in a native Windows session.
- Native Windows PowerPoint COM control has now passed for the current pre-title-fix T03 real candidate: `/real_product_outputs/powerpoint_runtime_t03/report.json` records three complete rounds, each exporting the 18-page candidate at 1600x900 plus the minimal-image control, with no residual PowerPoint process. The three candidate render rounds are byte-identical. Full contact-sheet review found no obvious clipping or image/text association defect on P007, P016, or the other slides. P013 exposed a wording mismatch, however: the source contains five compared models but uses four visible modules because the final two models share one comparison module. Its title was corrected from `五组模型` to `五个模型`, with regression coverage. The corrected candidate at `/tmp/academic-ppt-t03-powerpoint-title-v1` passes portable structural, semantic, composition, manifest, and object QA; it requires one fresh native PowerPoint render before T03 can receive a final visual-review decision.
- Fresh PowerPoint review of the title-fix candidate exposed a second P013 defect: `_process_heading` replaced every BERT-containing model label with `语义编码`. Explicit model identifiers now take precedence over generic semantic process labels. `/tmp/academic-ppt-t03-model-heading-v1` confirms the corrected four visible modules are `BiLSTM-CRF`, `BERT-CRF`, `BERT-BiLSTM-CRF`, and `GlobalPointer 与对抗训练`; it passes portable structural, semantic, composition, manifest, and object QA. Render this exact corrected candidate in native PowerPoint before recording the final T03 visual decision.
- The first native PowerPoint control run for the corrected model-heading candidate rendered P013 with the correct four module labels and completed its first 18-page candidate round, but failed the three-round stability contract when PowerPoint did not exit within the previous 10-second process-baseline wait after the second minimal-image control. This is correctly treated as a runtime-control failure, not an accepted render. COM export now clears both COM references, forces .NET garbage collection and finalization, and defaults to a 30-second configurable cleanup wait; a new native runtime-control invocation can validate the existing candidate without rebuilding it.
- The final T03 native runtime report at `real_product_outputs/powerpoint_runtime_t03_model_heading_v2/report.json` passes: three rounds each exported the exact 18-page candidate `real_product_outputs/liujinping_defense_t03_model_heading/working/final/complete_deck.pptx` at 1600x900, passed the minimal-image control, and returned PowerPoint to the zero-process baseline. The candidate hash is `5ff2a262c558d289d83eae5ae6d12e6c7b9db36130b93301ef6f9f74fcb9bd06`; P013's three PowerPoint PNGs are byte-identical and show the four intended module labels. This clears T03's authoritative runtime-stability gate only. Keep `product_accepted=false` until every final slide has an explicit full-resolution visual-review decision and the user confirms the completed deck.

## Release 1.0 Continuation (2026-08-06)

- The local branch was aligned with `origin/main`; the optional draw.io experiment is excluded from the 1.0 mainline. Adjacent architecture/process pages now avoid repeating the same visual grammar and semantic template archetype. Full regression passes with `153 passed`, and the Skill validator passes at 495 lines.
- `compile_bundled_source_templates.py` now compiles the complete T01-T08 surface by default. `references/compiled-template-report.json` records successful semantic compilation for all eight templates with no remaining template IDs. T01-T03 retain passed standard-template PowerPoint review; T04-T08 remain `pending` and are not release accepted.
- The native renderer no longer auto-accepts visual tasks. Both rendering paths leave tasks at `bound_to_slide` unless a complete per-slide visual-review file passes, closing a false-positive product-acceptance path.
- `references/release-evidence.json` and `scripts/audit_release_readiness.py` now define and audit the exact ten-scene/eight-template release surface. The audit binds sources, candidate PPTX files, build/object/composition reports, three-round PowerPoint reports, visual reviews, verification reports, and final confirmation by SHA-256. Missing or stale evidence keeps `release_accepted=false` with explicit blockers.
- The release matrix assigns T05/T06/T03/T01/T01/T07/T04/T04/T08/T02 across the ten canonical scenes, covering every bundled template while retaining a second T04 high-risk scene. The next stage must create the real weekly-progress source, generate all ten exact candidates, and produce the PowerPoint and visual-review evidence named by the manifest.
